import { Component, CUSTOM_ELEMENTS_SCHEMA, inject, model, OnInit, OnDestroy } from '@angular/core';
...
export class PlantDetailsComponent implements OnDestroy {
  ...
  private speechLanguage: string = 'en-IN';
  ...
  private flattenSpeechValue(value: any): string {
  ...
  private getSpeechText(): string {
    ...
    // For Bengali/Hindi/etc., the user should translate first.
    if (!isEnglish && !this.translatedDetails) {
      return '';
    }

    const translated = this.translatedDetails || {};

    const getValue = (field: string): any => {
      if (isEnglish) {
        return this.plantDetails[field];
      }

      return translated[field] ?? this.plantDetails[field];
    };

    const sections: string[] = [];

    // Keep plant/common name and botanical name from the original data.
    if (
      Array.isArray(this.plantDetails.commonNames) &&
      this.plantDetails.commonNames.length > 0
    ) {
      sections.push(
        `Plant name: ${this.plantDetails.commonNames.join(', ')}`
      );
    }

    if (this.plantDetails.botanicalName) {
      sections.push(
        `Botanical name: ${this.plantDetails.botanicalName}`
      );
    }

    const fields: Array<{ label: string; field: string }> = [
      { label: 'Family', field: 'family' },
      { label: 'Plant type', field: 'plantType' },
      { label: 'Description', field: 'description' },
      { label: 'Medicinal uses', field: 'medicinalUses' },
      { label: 'Chemical composition', field: 'composition' },
      { label: 'How to use', field: 'usageProcedure' },
      { label: 'Cultivation methods', field: 'cultivationMethods' },
      { label: 'Parts used', field: 'partsUsed' },
      { label: 'Usage time', field: 'usageTime' },
      { label: 'Category', field: 'category' },
      { label: 'Region', field: 'region' },
      { label: 'AYUSH type', field: 'AYUSHType' },
      { label: 'Precautions', field: 'precautions' }
    ];

    fields.forEach(({ label, field }) => {
      const value = getValue(field);
      const readableValue = this.flattenSpeechValue(value);

      if (readableValue.trim()) {
        sections.push(`${label}: ${readableValue}`);
      }
    });
  ...
  private createSpeechChunks(text: string): string[] {
    const maxLength = 180;
    const chunks: string[] = [];
    ...
    while (remainingText.length > maxLength) {
      ...
      chunks.push(
        remainingText.substring(0, splitIndex + 1).trim()
      );
  ...
  readPlantDetails(): void {
    ...
    if (!text) {
      if (this.selectedLanguage !== 'en') {
        alert('Please translate the plant details first.');
      } else {
        alert('No plant details available to read.');
      }
      return;
    }

    this.speechLanguage = this.getSpeechLanguage();
    this.speechChunks = this.createSpeechChunks(text);
  ...
  private speakNextChunk(): void {
    if (this.currentSpeechIndex >= this.speechChunks.length) {
    ...
    const utterance = new SpeechSynthesisUtterance(
      this.speechChunks[this.currentSpeechIndex]
    );
